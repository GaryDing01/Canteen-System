            ...
            <!-- <li class="li" v-for="(item,index) in menu" :key="index" :class="{active: index ===  currentIndex}" @click='itemClick(index)'>
              <span>{{item.name}}</span>
               ...
               <div class="freeTableButton">
                <el-button type="success" plain size="middle" style="height: 50px" @click="availableTable()">查看可用餐桌</el-button>
              </div>  
    </div>
...
export default {
  ...
  mounted(){
      this.$axios.get("/api/Dining_table/getAllTableState", {
        params: {
